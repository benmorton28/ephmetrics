#' @title Get Honors
#' @description Gets the percentage of honors from an archived Williams Course Catalog PDF link
#' @param link A link to a yearly pdf file on the Williams College Registrar's website.
#' @return A numeric vector containing percentage that graudated cum laude, magna cum laude and summa cum laude, in that order.
...
get_enrollment <- function(link= "http://web.williams.edu/admin/registrar//catalog/catalog0910.pdf"){
  ...
  replaced_text <- gsub(".*(Graduation with Distinction)","",replaced_text)
